...
function MakeMask(Dir, New_dir, Cell_mask, target_file_names, min_int_threshold){
	// cell_mask
	open(Dir + Cell_mask);
	...
	for(i=0; i<target_file_names.length+2; i++){
		setSlice(i+1);
		if(i==0){
		...
		if(i>1){
			run("Set Label...", "label=" + target_file_names[i-2]);
		}
	}
...
for(dir_idx=0; dir_idx<Dir_list.length; dir_idx++){
	...
	for(i=0; i<tif_file_names.length; i++){
		// 名前を予め記入
		if(endsWith(tif_file_names[i], "_polyfit2_2095.0-2120.0_0-0.tif")){
			target_file_name = tif_file_names[i];
			target_file_nickname_pre = split(tif_file_names[i], "_");
			target_file_nickname = "";
			for(j=1; j<target_file_nickname_pre.length - 6; j++){
				target_file_nickname += target_file_nickname_pre[j] + "_";
			}
			target_file_nickname += target_file_nickname_pre[j];
			N_target_file += 1;
		}
		if(endsWith(tif_file_names[i], "_signal_to_baseline_1900-2500_0-65535.tif")){
			LDarea_file_name = tif_file_names[i];
			N_LDarea_file += 1;
		}
		if(endsWith(tif_file_names[i], "_mask.tif")){
			mask_file = tif_file_names[i];
			N_mask_file += 1;
		}
	}
	if(N_target_file == 0){
	...
	if(N_target_file != 1){
		exit("WARNING: Multiple target files were detected!");
	}
	if(N_LDarea_file != 1){
		exit("WARNING: Number of LDarea files are incorrect!");
	}
	if(N_mask_file != 1){
		exit("WARNING: Number of N_mask files are incorrect!");
	}
	//
	BATCH = 0;
	setBatchMode(BATCH);
	...
	target_file_names = newArray(target_file_name);
	target_file_nicknames = newArray(target_file_nickname);
	LDarea_file_names = newArray(LDarea_file_name);
	Cell_mask_woExt = MakeMask(Dir, New_dir, mask_file, LDarea_file_names, min_int_threshold);
	// make LD_area: "_common2.tif", "_sumLD.tif" images -> "_LDarea.tif", "_CytoArea.tif" (32-bit with 0 or 1 values)
