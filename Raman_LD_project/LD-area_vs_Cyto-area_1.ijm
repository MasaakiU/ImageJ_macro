...
d_FA_list = newArray("pseudo", "d14aLA", "d8AA", "d5DHA");
...
function CalculateRatio(Dir, New_dir, target_file_names, target_file_nicknames, Cell_mask_woExt){
	...
	for(i=0; i<target_file_names.length; i++){
		...
		for(j=0; j<target_file_names.length; j++){
			...
			setMinAndMax(0, 5);
			run("Fire");
			saveAs("Tiff", New_dir + i_nickname + "_by_" + j_nickname + ".tif");
			imageCalculator("Multiply", i_nickname + "_by_" + j_nickname + ".tif", Cell_mask_woExt + "_common2.tif");
...
Dialog.create("Parameter Settings");
	...
	for(i=0; i<tif_file_names.length; i++){
		// 名前を予め記入
		isCategolized = false;
		if(endsWith(tif_file_names[i], "cyto.tif")){
			Dialog.addString(tif_file_names[i], "");
			mask_candidate = tif_file_names[i];
			isCategolized = true;
		}
		if(endsWith(tif_file_names[i], "_unmixed_1900-2400_1_0-65535.tif")){
			Dialog.addString(tif_file_names[i], d_FA_list[1]);
			isCategolized = true;
		}
		if(endsWith(tif_file_names[i], "_unmixed_1900-2400_2_0-65535.tif")){
			Dialog.addString(tif_file_names[i], d_FA_list[2]);
			isCategolized = true;
		}
		if(endsWith(tif_file_names[i], "_unmixed_1900-2400_3_0-65535.tif")){
			Dialog.addString(tif_file_names[i], d_FA_list[3]);
			isCategolized = true;
		}
		if(endsWith(tif_file_names[i], "_unmixed_450-680_1_0-65535.tif")){
			Dialog.addString(tif_file_names[i], BrFA_list[1]);
			isCategolized = true;
		}
		// どこにも所属しなかった場合
		if(!isCategolized){
			//Dialog.addString(tif_file_names[i], "Name" + file_idx);
			Dialog.addString(tif_file_names[i], "");
			file_idx += 1;
		}
	}
...
Cell_mask = Dialog.getChoice();
min_int_threshold = Dialog.getNumber();
Med_filter_size = Dialog.getNumber();
Threshold_metric = Dialog.getChoice();
LD_margin = Dialog.getNumber();
