...
minimum_intensity_to_use = 25;  //10
...
Dialog.create("Parameter Settings");
	...
	for(i=0; i<tif_file_names.length; i++){
		...
		if(endsWith(tif_file_names[i], "_unmixed_1900-2400_4_0-65535.tif")){
			Dialog.addString(tif_file_names[i], d_FA_list[4]);
			isCategolized = true;
		}
    ...
    Dialog.addNumber("Minimum intensity to use", minimum_intensity_to_use);
    Dialog.addMessage(" ");
    // LD area definition settings
	Dialog.addMessage("=== LD definition settings ===");
    Dialog.addNumber("Median Filter Size", median_filter_size);
	Dialog.addChoice("Threshold Metric", ThresholdMetric);
    Dialog.addNumber("LD_margin", ld_margin);
